...
**ownerSupplied.options[]** — Each owner-supplied item gains an `options` array. Each option contains: supplier, description, price, and an `active` boolean. Only the active option feeds into budget calculations. Example: kitchen worktop could have Option A (Howdens, laminate, £2,400), Option B (Benchmarx, quartz, £4,100), Option C (bespoke, dekton, £6,800) — user selects which is in effect.

**ownerSupplied auto-population** — When a spec document is uploaded and parsed by AI, owner-supplied items are automatically extracted and populated. User can then add new items manually that weren't in the spec.
...
The app will use the Anthropic API (via the Claude artifact API pattern — fetch to `api.anthropic.com/v1/messages` using `claude-sonnet-4-20250514`) to:
...
| GBP currency | User is UK-based (Southall, England) |
| Client-side Anthropic API | No backend to manage, runs in browser |
...
- **Awaiting:** User's actual spec document and builder tenders for real data population
